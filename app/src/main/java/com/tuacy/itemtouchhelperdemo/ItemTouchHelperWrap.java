...
public class ItemTouchHelperWrap extends RecyclerView.ItemDecoration implements RecyclerView.OnChildAttachStateChangeListener {
	...
	/**
	 * ItemTouchHelperWrap is in idle state. At this state, either there is no related motion event by
	...
	static final String TAG = "ItemTouchHelperWrap";
	...
	/**
	 * Set when ItemTouchHelperWrap is assigned to a RecyclerView.
	 */
	...
	/**
	 * Developer callback which controls the behavior of ItemTouchHelperWrap.
	 */
	Callback mCallback;
	...
	/**
	 * Creates an ItemTouchHelperWrap that will work with the given Callback.
	 * <p>
	 * You can attach ItemTouchHelperWrap to a RecyclerView via
	 * {@link #attachToRecyclerView(RecyclerView)}. Upon attaching, it will add an item decoration,
	 * an onItemTouchListener and a Child attach / detach listener to the RecyclerView.
	 *
	 * @param callback The Callback which controls the behavior of this touch helper.
	 */
	public ItemTouchHelperWrap(Callback callback) {
		mCallback = callback;
	}
	...
	/**
	 * Attaches the ItemTouchHelperWrap to the provided RecyclerView. If TouchHelper is already
	 * attached to a RecyclerView, it will first detach from the previous one. You can call this
	 * method with {@code null} to detach it from the current RecyclerView.
	 *
	 * @param recyclerView The RecyclerView instance to which you want to add this helper or {@code null} if you want to remove
	 *                     ItemTouchHelperWrap from the current RecyclerView.
	 */
	...
	private void initGestureDetector() {
		...
		mGestureDetector = new GestureDetectorCompat(mRecyclerView.getContext(), new ItemTouchHelperCompGestureListener());
	}
	 ...
	 *
	 * @param selected    The ViewHolder to drag or swipe. Can be null if you want to cancel the current action
	...
	void select(ViewHolder selected, int actionState) {
		...
		if (mSelected != null) {
			...
			if (prevSelected.itemView.getParent() != null) {
				...
				final RecoverAnimation rv = new RecoverAnimation(prevSelected, animationType, prevActionState, currentTranslateX,
				...
				final long duration = mCallback.getAnimationDuration(mRecyclerView, animationType, targetTranslateX - currentTranslateX,
	...
	void postDispatchSwipe(final RecoverAnimation anim, final int swipeDir) {
		...
		mRecyclerView.post(new Runnable() {
			@Override
			public void run() {
				if (mRecyclerView != null && mRecyclerView.isAttachedToWindow() && !anim.mOverridden &&
	...
	void moveIfNecessary(ViewHolder viewHolder) {
		...
		if (Math.abs(y - viewHolder.itemView.getTop()) < viewHolder.itemView.getHeight() * threshold &&
			Math.abs(x - viewHolder.itemView.getLeft()) < viewHolder.itemView.getWidth() * threshold) {
			return;
		}
	...
	/**
	 * Starts dragging the provided ViewHolder. By default, ItemTouchHelperWrap starts a drag when a
	 * View is long pressed. You can disable that behavior by overriding
	 * {@link ItemTouchHelperWrap.Callback#isLongPressDragEnabled()}.
	 * <p>
	 ...
	 * ItemTouchHelperWrap
	 * is attached.</li>
	 * <li>{@link ItemTouchHelperWrap.Callback} must have dragging enabled.</li>
	 * <li>There must be a previous touch event that was reported to the ItemTouchHelperWrap
	 ...
	 *                 mItemTouchHelperComp.startDrag(viewHolder);
	 *             }
	 *             return false;
	 *         }
	 *     });
	 ...
	 * @see ItemTouchHelperWrap.Callback#isItemViewSwipeEnabled()
	 */
	public void startDrag(ViewHolder viewHolder) {
		...
		if (viewHolder.itemView.getParent() != mRecyclerView) {
			Log.e(TAG, "Start drag has been called with a view holder which is not a child of " +
					   "the RecyclerView which is controlled by this ItemTouchHelperWrap.");
			return;
		}
		obtainVelocityTracker();
		mDx = mDy = 0f;
	...
	/**
	 * Starts swiping the provided ViewHolder. By default, ItemTouchHelperWrap starts swiping a View
	 * when user swipes their finger (or mouse pointer) over the View. You can disable this
	 * behavior
	 * by overriding {@link ItemTouchHelperWrap.Callback}
	 * <p>
	 * For this method to work:
	 * <ul>
	 * <li>The provided ViewHolder must be a child of the RecyclerView to which this
	 * ItemTouchHelperWrap is attached.</li>
	 * <li>{@link ItemTouchHelperWrap.Callback} must have swiping enabled.</li>
	 * <li>There must be a previous touch event that was reported to the ItemTouchHelperWrap
	 * through RecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
	 * grabs previous events, this should work as expected.</li>
	 * </ul>
	 ...
	 *         public boolean onTouch(View v, MotionEvent event) {
	 *             if (MotionEvent.getActionMasked(event) == MotionEvent.ACTION_DOWN) {
	 *                 mItemTouchHelperComp.startSwipe(viewHolder);
	 *             }
	 *             return false;
	 *         }
	 *     });
	...
	public void startSwipe(ViewHolder viewHolder) {
		...
		if (viewHolder.itemView.getParent() != mRecyclerView) {
			Log.e(TAG, "Start swipe has been called with a view holder which is not a child of " +
					   "the RecyclerView controlled by this ItemTouchHelperWrap.");
			return;
		}
		obtainVelocityTracker();
		mDx = mDy = 0f;
		...
		if ((flags & (LEFT | RIGHT)) != 0) {
			...
			if (mVelocityTracker != null && mActivePointerId > -1) {
				...
				if ((velDirFlag & flags) != 0 && dirFlag == velDirFlag &&
					absXVelocity >= mCallback.getSwipeEscapeVelocity(mSwipeEscapeVelocity) && absXVelocity > Math.abs(yVelocity)) {
					return velDirFlag;
				}
			}
		...
		if ((flags & (UP | DOWN)) != 0) {
			...
			if (mVelocityTracker != null && mActivePointerId > -1) {
				...
				if ((velDirFlag & flags) != 0 && velDirFlag == dirFlag &&
					absYVelocity >= mCallback.getSwipeEscapeVelocity(mSwipeEscapeVelocity) && absYVelocity > Math.abs(xVelocity)) {
					return velDirFlag;
				}
			}
	 ...
	 * {@link ItemTouchHelperWrap}.
	 */
	public interface ViewDropHandler {

		/**
		 * Called by the {@link ItemTouchHelperWrap} after a View is dropped over another View.
		 * <p>
		 ...
		 *
		 * @param view   The View which is being dragged. It is very likely that user is still dragging this View so there might be other
		 *               {@link #prepareForDrop(View, View, int, int)} after this one.
		 * @param target The target view which is being dropped on.
		 * @param x      The <code>left</code> offset of the View that is being dragged. This value includes the movement caused by the
		 *               user.
		 * @param y      The <code>top</code> offset of the View that is being dragged. This value includes the movement caused by the
		 *               user.
		 */
	...
	/**
	 * This class is the contract between ItemTouchHelperWrap and your application. It lets you control
	 ...
	 * {@link SimpleCallback}.
	 * <p>
	 * If user drags an item, ItemTouchHelperWrap will call
	 ...
	 * custom drop target.
	 * <p>
	 * When a View is swiped, ItemTouchHelperWrap animates it until it goes out of bounds, then calls
	...
	public abstract static class Callback {
		...
		private static final int ABS_HORIZONTAL_DIR_FLAGS = LEFT | RIGHT | ((LEFT | RIGHT) << DIRECTION_FLAG_COUNT) |
		...
		static {
			sUICallback = new ItemTouchUIUtilWrap();
		}
		 ...
		 * @param layoutDirection The layout direction of the View. Can be obtained from {@link ViewCompat#getLayoutDirection(android.view.View)}.
		 * @return Updated flags which uses relative flags ({@link #START}, {@link #END}) instead of {@link #LEFT}, {@link #RIGHT}.
		...
		public static int makeMovementFlags(int dragFlags, int swipeFlags) {
			return makeFlag(ACTION_STATE_IDLE, swipeFlags | dragFlags) | makeFlag(ACTION_STATE_SWIPE, swipeFlags) |
		 ...
		 *
		 * @param actionState The action state you want to get flags in. Should be one of {@link #ACTION_STATE_IDLE}, {@link
		 *                    #ACTION_STATE_SWIPE} or {@link #ACTION_STATE_DRAG}.
		 * @param directions  The direction flags. Can be composed from {@link #UP}, {@link #DOWN}, {@link #RIGHT}, {@link #LEFT} {@link
		 *                    #START} and {@link #END}.
		 ...
		 *
		 * @param recyclerView The RecyclerView to which ItemTouchHelperWrap is attached to.
		 * @param current      The ViewHolder that user is dragging.
		 * @param target       The ViewHolder which is below the dragged ViewHolder.
		 * @return True if the dragged ViewHolder can be replaced with the target ViewHolder, false otherwise.
		 */
		...
		/**
		 * Called when ItemTouchHelperWrap wants to move the dragged item from its old position to
		 * the new position.
		 * <p>
		 * If this method returns true, ItemTouchHelperWrap assumes {@code viewHolder} has been moved
		...
		/**
		 * Returns whether ItemTouchHelperWrap should start a drag and drop operation if an item is
		 * long pressed.
		 * <p>
		 * Default value returns true but you may want to disable this if you want to start
		 * dragging on a custom view touch using {@link #startDrag(ViewHolder)}.
		 *
		 * @return True if ItemTouchHelperWrap should start dragging an item when it is long pressed, false otherwise. Default value is
		 * <code>true</code>.
		...
		/**
		 * Returns whether ItemTouchHelperWrap should start a swipe operation if a pointer is swiped
		 * over the View.
		 * <p>
		 * Default value returns true but you may want to disable this if you want to start
		 * swiping on a custom view touch using {@link #startSwipe(ViewHolder)}.
		 *
		 * @return True if ItemTouchHelperWrap should start swiping an item when user swipes a pointer over the View, false otherwise.
		...
		/**
		 * When finding views under a dragged view, by default, ItemTouchHelperWrap searches for views
		 ...
		 * dragged. After a view is moved this amount, ItemTouchHelperWrap starts checking for Views
		 ...
		 * Keep in mind that ItemTouchHelperWrap also checks the perpendicular velocity and makes sure
		 ...
		 * multiplier.
		 *
		 * @param defaultValue The default value (in pixels per second) used by the ItemTouchHelperWrap.
		 * @return The minimum swipe velocity. The default implementation returns the <code>defaultValue</code> parameter.
		...
		/**
		 * Defines the maximum velocity ItemTouchHelperWrap will ever calculate for pointer movements.
		 * <p>
		 * To consider a movement as swipe, ItemTouchHelperWrap requires it to be larger than the
		 ...
		 *
		 * @param defaultValue The default value(in pixels per second) used by the ItemTouchHelperWrap.
		 * @return The velocity cap for pointer movements. The default implementation returns the <code>defaultValue</code> parameter.
		...
		/**
		 * Called by ItemTouchHelperWrap to select a drop target from the list of ViewHolders that
		 ...
		 * @param dropTargets The list of ViewHolder that are under the dragged View and candidate as a drop.
		 * @param curX        The updated left value of the dragged View after drag translations are applied. This value does not include
		 *                    margins added by {@link RecyclerView.ItemDecoration}s.
		 * @param curY        The updated top value of the dragged View after drag translations are applied. This value does not include
		 *                    margins added by {@link RecyclerView.ItemDecoration}s.
		 ...
		 * <p>
		 * ItemTouchHelperWrap will keep a reference to the View until it is detached from
		 * RecyclerView.
		 * As soon as it is detached, ItemTouchHelperWrap will call
		 * {@link #clearView(RecyclerView, ViewHolder)}.
		 *
		 * @param viewHolder The ViewHolder which has been swiped by the user.
		 * @param direction  The direction to which the ViewHolder is swiped. It is one of {@link #UP}, {@link #DOWN}, {@link #LEFT} or
		 *                   {@link #RIGHT}. If your {@link #getMovementFlags(RecyclerView, ViewHolder)} method returned relative flags
		 *                   instead of {@link #LEFT} / {@link #RIGHT}; `direction` will be relative as well. ({@link #START} or {@link
		 *                   #END}).
		 */
		public abstract void onSwiped(ViewHolder viewHolder, int direction);

		/**
		 * Called when the ViewHolder swiped or dragged by the ItemTouchHelperWrap is changed.
		 * <p/>
		 * If you override this method, you should call super.
		 *
		 * @param viewHolder  The new ViewHolder that is being swiped or dragged. Might be null if it is cleared.
		 * @param actionState One of {@link ItemTouchHelperWrap#ACTION_STATE_IDLE}, {@link ItemTouchHelperWrap#ACTION_STATE_SWIPE} or {@link
		 *                    ItemTouchHelperWrap#ACTION_STATE_DRAG}.
		 ...
		 * <p>
		 * ItemTouchHelperWrap does not create an extra Bitmap or View while dragging, instead, it
		 ...
		 *
		 * @param recyclerView The RecyclerView controlled by the ItemTouchHelperWrap.
		 * @param viewHolder   The ViewHolder under user's control.
		 * @param fromPos      The previous adapter position of the dragged item (before it was moved).
		 * @param target       The ViewHolder on which the currently active item has been dropped.
		 * @param toPos        The new adapter position of the dragged item.
		 * @param x            The updated left value of the dragged View after drag translations are applied. This value does not include
		 *                     margins added by {@link RecyclerView.ItemDecoration}s.
		 * @param y            The updated top value of the dragged View after drag translations are applied. This value does not include
		 *                     margins added by {@link RecyclerView.ItemDecoration}s.
		 */
		...
		void onDraw(Canvas c,
					RecyclerView parent,
					ViewHolder selected,
					List<ItemTouchHelperWrap.RecoverAnimation> recoverAnimationList,
					int actionState,
					float dX,
					float dY) {
			final int recoverAnimSize = recoverAnimationList.size();
			for (int i = 0; i < recoverAnimSize; i++) {
				final ItemTouchHelperWrap.RecoverAnimation anim = recoverAnimationList.get(i);
				anim.update();
		...
		/**
		 * Called by ItemTouchHelperWrap on RecyclerView's onDraw callback.
		 * <p>
		 ...
		 * <code>dY</code>.
		 * ItemTouchHelperWrap also takes care of drawing the child after other children if it is being
		 ...
		 * @param recyclerView      The RecyclerView to which ItemTouchHelperWrap is attached to
		 * @param viewHolder        The ViewHolder which is being interacted by the User or it was interacted and simply animating to its
		 *                          original position
		 * @param dX                The amount of horizontal displacement caused by user's action
		 * @param dY                The amount of vertical displacement caused by user's action
		 * @param actionState       The type of interaction on the View. Is either {@link #ACTION_STATE_DRAG} or {@link
		 *                          #ACTION_STATE_SWIPE}.
		 * @param isCurrentlyActive True if this view is currently being controlled by the user or false it is simply animating back to its
		 *                          original state.
		...
		/**
		 * Called by the ItemTouchHelperWrap when user action finished on a ViewHolder and now the View
		 ...
		 *
		 * @param recyclerView  The RecyclerView to which the ItemTouchHelperWrap is attached to.
		 * @param animationType The type of animation. Is one of {@link #ANIMATION_TYPE_DRAG}, {@link #ANIMATION_TYPE_SWIPE_CANCEL} or
		...
		/**
		 * Called by the ItemTouchHelperWrap when user is dragging a view out of bounds.
		 * <p>
		 ...
		 *
		 * @param recyclerView        The RecyclerView instance to which ItemTouchHelperWrap is attached to.
		 * @param viewSize            The total size of the View in scroll direction, excluding item decorations.
		 * @param viewSizeOutOfBounds The total size of the View that is out of bounds. This value is negative if the View is dragged
		 *                            towards left or top edge.
		 ...
		 * @return The amount that RecyclerView should scroll. Keep in mind that this value will be passed to {@link
		 * RecyclerView#scrollBy(int, int)} method.
		 */
	 ...
	 *
	 * <pre>
	 * ItemTouchHelperWrap mIth = new ItemTouchHelperWrap(
	 *     new ItemTouchHelperWrap.SimpleCallback(ItemTouchHelperWrap.UP | ItemTouchHelperWrap.DOWN,
	 *         ItemTouchHelperWrap.LEFT) {
	...
	public abstract static class SimpleCallback extends Callback {
		 ...
		 *
		 * @param dragDirs  Binary OR of direction flags in which the Views can be dragged. Must be composed of {@link #LEFT}, {@link
		 *                  #RIGHT}, {@link #START}, {@link #END}, {@link #UP} and {@link #DOWN}.
		 * @param swipeDirs Binary OR of direction flags in which the Views can be swiped. Must be composed of {@link #LEFT}, {@link
		 *                  #RIGHT}, {@link #START}, {@link #END}, {@link #UP} and {@link #DOWN}.
		 */
	...
	private class ItemTouchHelperCompGestureListener extends GestureDetector.SimpleOnGestureListener {

		ItemTouchHelperCompGestureListener() {
		}
	...
	private static class RecoverAnimation implements Animator.AnimatorListener {
		...
		RecoverAnimation(ViewHolder viewHolder,
						 int animationType,
						 int actionState,
						 float startDx,
						 float startDy,
