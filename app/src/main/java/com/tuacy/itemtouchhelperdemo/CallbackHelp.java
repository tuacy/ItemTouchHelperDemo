...
public abstract class CallbackHelp extends ItemTouchHelper.Callback {
	...
	/**
	 * 针对swipe和drag状态，获取不同状态(swipe、drag)下可以移动的方向
	...
	/**
	 * 针对drag状态，当前target对应的item view是否可以加入drag状态
	 * (换句话说我们一般用drag来做一些换位置的操作，就是当前target对应的item view 是否可以换位置)
	 */
	...
	/**
	 * 针对drag状态，在canDropOver()函数返回true的情况下，会调用该函数让我们去处理拖动换位子的流程，从旧位置拖动到新位置
	...
	/**
	 * 针对drag状态，当drag itemView和底下的itemView重叠的时候,可以给drag itemView设置额外的margin，让重叠更加容易发生。相当于增大drag itemView的区域
	 */
	...
	/**
	 * 针对swipe状态，swipe滑动的位置超过了百分值多少就消失
	 */
	...
	/**
	 * swipe的逃逸速度，换句话说就算没达到getSwipeThreshold的设置，只要达到了这个速度 item也会被swipe消失掉
	 */
	...
	/**
	 * swipe 到达滑动消失的距离回调函数,一般在这个函数里面处理删除item view的逻辑(确切的来讲是到达swipe小时的位置的动画结束的时候调用)
	 */
	 ...
	 * drag状态：当手指释放的时候会调用
	 * swipe状态：当item view从RecyclerView中删除的时候调用，一般会在onSwiped()函数里面删除掉指定的item view
	 */
